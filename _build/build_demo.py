...
h = rep1("var HKEY='cbt_history_v1', PKEY='cbt_progress_v1', SKEY='cbt_seen_v1';",
         "var HKEY='cbt_history_demo', PKEY='cbt_progress_demo', SKEY='cbt_seen_demo';", h)
...
LOCK = ('<section class="subject" id="lock">'
        ...
        '<p><b>전체판 구성</b> — 3개 과목 개념집 + <b>400문항 모의고사</b>(자동 채점·합격 판정·오답 해설·응시 이력).</p>'
        '<p>구매·문의: <b>' + EMAIL + '</b></p>'
        '</div></div></section>')
...
ribbon = ('<div id="demoRibbon">\U0001F381 맛보기 버전 — 제1과목 개념 + 샘플 문항만 열려 있어요 · '
          '전체(3과목 + 400문항)는 구매 시 제공 · 문의 <a href="mailto:' + EMAIL + '">' + EMAIL + '</a></div>')
h = rep1('<header class="top">', ribbon + '<header class="top">', h)

# --- 모의고사 안내문을 맛보기용으로 ---
h = rep1('개념집 내용을 바탕으로 한 4지선다 문제를 실전처럼 풀고 채점받으세요. 응시 이력은 이 브라우저(localStorage)에만 저장되며 외부로 전송되지 않습니다. 시험 도중 새로고침하거나 창을 닫아도 이어서 풀 수 있습니다.',
         '맛보기 샘플 12문항으로 채점·해설을 체험해 보세요. 전체판은 400문항으로 실전(과목당 20문항)까지 지원합니다. 응시 이력은 이 브라우저에만 저장됩니다.', h)
...
assert "cbt_seen_v1" not in h and "cbt_history_v1" not in h and "cbt_progress_v1" not in h, "전체판 localStorage 키 잔존"
...
print("verify: 문항 12/12, 2·3과목 섹션 제거, localStorage 분리, 유출 0건 — OK")
